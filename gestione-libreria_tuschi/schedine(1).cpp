...
void gestioneEventi(int minuto, partita& p, squadra& s1, squadra& s2) {
	...
	if (evento < 5) {
		...
		if (chance < goalProb * 0.6f) {
			cout << " GOL!" << endl;
			p.goal1++;
		} else if (chance < goalProb * 0.9f) {
...
void classifica(vector<risultati>& sts, vector<squadra>& squ, int dim){
	vector<squadra> ordinamento(dim);
	
	for(int i = 0; i < dim; i++){
		squ.punteggio = sts[i].vittorie + (sts[i].goal - sts[i].goal_presi);
		ordinamento.push_back(squ.punteggio);
	}
}
