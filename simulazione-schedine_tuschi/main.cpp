...
void gestioneEventi(int minuto, partita& p, squadra& s1, squadra& s2) {
	...
	if (evento < 5) {
		...
		sleep(1);
		cout << "[" << minuto << "''] Giocata tecnica del " << s2.nome << ", ma c'� fuorigioco!" << endl;
	} else if (evento < 68) {
		sleep(1);
	...
	// 26% di probabilit� che non succeda nulla
}
...
vector<risultati> creaClassifica(vector<risultati> ris){
    vector<risultati> clas;
    risultati punteggio_squadra;
    
    for(const risultati& i : ris){
        punteggio_squadra.punteggio = i.vittorie + (i.goal - i.goal_presi);
        punteggio_squadra.nome = i.nome;
        
        clas.push_back(punteggio_squadra);
    }
    
    sort(clas.begin(), clas.end(), [](risultati& r1, risultati& r2){
        return r1.punteggio < r2.punteggio;
    });
...
int main() {
	...
	vector<risultati> Classifica(numero_squadre);
	...
	vector<risultati> classifica = creaClassifica(stats);
	int contatore = 1;
	
	// Stampa classifica
	cout << "\n===== CLASSIFICA =====" << endl;
	for(const risultati& r : classifica){
	    if(contatore<3)cout<<contatore<<"# "<<r.nome<<endl;
	    else cout<<r.nome<<endl;
	    contatore++;
	}
	contatore = 1;
