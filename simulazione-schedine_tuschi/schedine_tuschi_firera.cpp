...
int generaGoal(float fattore) {
    float probabilita_zero = 0.5f - fattore * 0.4f;
    float probabilita_doppietta = fattore * 0.6f;

    int goal = 0;
    float r = static_cast<float>(rand()) / RAND_MAX;

    if (r > probabilita_zero) {
        goal = 1;
        float r2 = static_cast<float>(rand()) / RAND_MAX;
        if (r2 < probabilita_doppietta) goal += rand() % 3; // +0 a +2
    }

    return std::min(goal, 5);
}


void simulaPartita(partita& p, const squadra& s1, const squadra& s2) {
    float fattore1 = s1.overall / float(s1.overall + s2.overall);
    float fattore2 = 1.0f - fattore1;

    // Goal realistici
    p.goal1 = generaGoal(fattore1);
    p.goal2 = generaGoal(fattore2);

    // Tiri dipendenti dai goal: almeno +5 tiri rispetto ai goal
    p.tiri1 = p.goal1 * (2 + rand() % 3) + 5 + rand() % 4;
    p.tiri2 = p.goal2 * (2 + rand() % 3) + 5 + rand() % 4;

    // Calci d'angolo in base al fattore + casualità
    p.angolo1 = rand() % (int)(fattore1 * 8 + 3);
    p.angolo2 = rand() % (int)(fattore2 * 8 + 3);

    // Gialli più realistici
    p.gialli1 = (rand() % 100 < (int)((1.0f - fattore1) * 70)) ? rand() % 3 : 0;
    p.gialli2 = (rand() % 100 < (int)((1.0f - fattore2) * 70)) ? rand() % 3 : 0;

    // Rossi: molto rari (5% probabilità base, leggermente aumentata se squadra debole)
    p.rossi1 = (rand() % 100 < (int)((1.0f - fattore1) * 8)) ? 1 : 0;
    p.rossi2 = (rand() % 100 < (int)((1.0f - fattore2) * 8)) ? 1 : 0;
}
